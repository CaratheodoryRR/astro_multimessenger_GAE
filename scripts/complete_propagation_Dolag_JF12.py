...
def run(
    ...
    if rnd:
        srcType = 'random'
    elif str(srcPath).endswith('.raw') or str(srcPath).endswith('.dat'):
        srcType = 'grid'
    else:
        srcType = 'point-like'
...
def run_extra_galactic_part(filesDict, kwargsProp, srcType, partNum, kappa, **kwargsSim):
    ...
    # Observer
    EG_obs = Observer()
    EG_obs.add(ObserverSurface( Sphere(galCenSGVec, rGalaxy) ))
    ...
    test_obs.add( MaximumTrajectoryLength( cpf.maxTrajectoryL ) )
...
def run_galactic_part(filesDict, rObs, **kwargsSim):
    ...
    input = ParticleCollector(0)
    parts = len(outputs)
    for i, (output, EGFile) in enumerate(zip(outputs, filesDict['extra-galactic'])):
        print(f'\n\tRUNNING PART {i+1} OF {parts}\n')
        input.load(EGFile)
        collector_coord_transformation(collector=input)
...
def main(args):
    ...
    if args.prop != 'galactic': 
        ...
        if str(args.dolagPath).endswith('.raw'):
            Dolag_field = setting_dolag_field(pathToDolag=args.dolagPath, bFactor=args.bFactor)
        elif str(args.dolagPath).endswith('.dat'):
            Dolag_field = setting_hackstein_field(pathToHackstein=args.dolagPath, bFactor=args.bFactor)
        print('Done!\n')
    else:
        Dolag_field = None
